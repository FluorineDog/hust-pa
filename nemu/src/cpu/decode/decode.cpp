...
make_DHelper(J) {
  ...
  g_decoding.jmp_eip = id_dest->simm + *eip;
  // DOG FUCK IT
  id_dest->val = g_decoding.jmp_eip;
}
...
make_DHelper(M2M) {
  ...
  rtl_mv(&id_src->addr, &cpu.esi);
  assert(id_dest->width == 1);
#ifdef DEBUG
  sprintf(id_dest->str, "(%%edi)");
  sprintf(id_src->str,  "(%%esi)");
#endif
}
