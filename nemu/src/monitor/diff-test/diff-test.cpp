...
void difftest_step(uint32_t eip) {
    ...
    int failed = false;
    for(int i = 0; i < 8; ++i) {
        auto ref_value = ref_r.gpr[i]._32;
    ...
    // failed = failed || ((cpu.eflags ^ ref_r.eflags) & test_flags) == 0)

    if(failed) {
        for(int i = 0; i < 8; ++i) {
            auto value = cpu.gpr[i]._32;
            auto ref = ref_r.gpr[i]._32;
            printf("%-8s0x%08x%16d [0x%08x%16d]\n", regsl[i], value, value, ref, ref);
        }
        printf("%-8s0x%08x%16d\n", "eip", cpu.eip, cpu.eip);
        printf("ZF=%d[%d], SF=%d[%d], CF=%d[%d], OF=%d[%d]\n", get_ZF(cpu.eflags),
               get_ZF(ref_r.eflags), get_SF(cpu.eflags), get_SF(ref_r.eflags),
               get_CF(cpu.eflags), get_CF(ref_r.eflags), get_OF(cpu.eflags),
               get_OF(ref_r.eflags));
    }
