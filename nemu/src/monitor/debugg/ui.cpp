...
char *rl_gets() {
    ...
    fprintf(stderr, "[%ld]", g_nr_guest_instr);
    auto new_line_read = readline("(nemu) ");
    if(new_line_read && *new_line_read) {
        if(line_read) {
            free(line_read);
            line_read = nullptr;
        }
...
void info_register() {
    ...
    printf("ZF=%d, SF=%d, CF=%d, OF=%d, IF=%d\n", get_ZF(cpu.eflags), get_SF(cpu.eflags),
           get_CF(cpu.eflags), get_OF(cpu.eflags), get_IF(cpu.eflags));
}
