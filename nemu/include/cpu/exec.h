...
#ifdef DEBUG
#define print_asm(...) \
    Assert(snprintf(g_decoding.assembly, 80, __VA_ARGS__) < 80, "buffer overflow!")
#else
#define print_asm(...)
#endif
...
#define print_asm_template3(instr)                                                \
    print_asm(str(instr) "%c %s,%s,%s", suffix_char(id_dest->width), id_src->str, \
              id_src2->str, id_dest->str)
