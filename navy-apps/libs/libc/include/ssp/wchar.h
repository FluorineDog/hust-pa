...
#define __ssp_redirect_wc(rtype, fun, args, call, cond, bos)        \
    __ssp_decl(rtype, fun, args) {                                  \
        if(cond) __ssp_check(__buf, __wlen * sizeof(wchar_t), bos); \
...
#define __ssp_bos_wicheck2_restrict(fun)                                        \
    __ssp_decl(wchar_t *, fun,                                                  \
               (wchar_t * __restrict __buf, const wchar_t *__restrict __src)) { \
...
__ssp_bos_wicheck2_restrict(wcpcpy) __ssp_bos_wicheck3_restrict(wcpncpy)
#endif
    __ssp_bos_wicheck2_restrict(wcscpy) __ssp_bos_wicheck2_restrict(wcscat)
        __ssp_bos_wicheck3_restrict(wcsncpy) __ssp_bos_wicheck3_restrict(wcsncat)
            __ssp_bos_wicheck3_restrict(wmemcpy) __ssp_bos_wicheck3(wmemmove)
#if __GNU_VISIBLE
                __ssp_bos_wicheck3_restrict(wmempcpy)
                    ...
                    __ssp_redirect_wc(wchar_t *, wmemset,
                        ...
                        __ssp_decl(size_t, wcrtomb,
                                   (char *__buf, wchar_t __src, mbstate_t *__ps)) {
    if(__buf != NULL && __src != L'\0') __ssp_check(__buf, sizeof(wchar_t), __ssp_bos);
...
__ssp_redirect_wc(size_t, mbsrtowcs,
    ...
    __ssp_redirect_raw(size_t, wcsrtombs,
        ...
        __ssp_redirect_wc(size_t, mbsnrtowcs,
                          (wchar_t * __buf, const char **__src, size_t __nms,
                           size_t __wlen, mbstate_t *__ps),
                          (__buf, __src, __nms, __wlen, __ps), __buf != NULL, __ssp_bos)

            __ssp_redirect_raw(size_t, wcsnrtombs,
                               (char *__buf, const wchar_t **__src, size_t __nwc,
                                size_t __len, mbstate_t *__ps),
                               (__buf, __src, __nwc, __len, __ps), __buf != NULL,
                               __ssp_bos)
#endif

                __ssp_decl(wchar_t *, fgetws,
                           (wchar_t * __restrict __buf, int __wlen,
                            __FILE *__restrict __fp)) {
    if(__wlen > 0) __ssp_check(__buf, (size_t)__wlen * sizeof(wchar_t), __ssp_bos);
