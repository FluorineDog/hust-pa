...
#else
    __dprintf(fmt, va_alist) char *fmt;
va_dcl
#endif
{
    va_list args;
    ...
    while(*fmt) {
        ...
        if(*fmt == 'N') {
            ...
            while(--count >= 0) {
                switch(c) {
                    case 'c': write_string(unctrl(*p++)); break;
                    case 'p':
    ...
    switch(size) {
        ...
        case 2:
            if(big_endian_p)
                ...
                x = (p[1] << 8) | p[0];
            if(!unsigned_p) x = (x ^ 0x8000) - 0x8000;
            return x;
        case 4:
            if(big_endian_p)
            ...
            if(!unsigned_p) x = (x ^ 0x80000000L) - 0x80000000L;
            return x;
