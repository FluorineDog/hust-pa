...
const wchar_t *__ctloc(wchar_t *buf, const char *elem, size_t *len_ret) {
    ...
    *len_ret = mbstowcs(buf, elem, CTLOCBUFLEN - 1);
    if(*len_ret == (size_t)-1) *len_ret = 0;
    return buf;
}
#define _ctloc(x) (ctloc = __ctloc(ctlocbuf, _CurrentTimeLocale->x, &ctloclen))
#endif
#endif /* MAKE_WCSFTIME */
...
#define ERA_STRTOL(a, b, c) strtol((a), (b), (c))
    ...
    while(era) {
        ...
        if(stm.tm_year <= -YEAR_BASE) ++stm.tm_year;
        ...
        if(era[0] == '-' && era[1] == '*') {
            ...
            /* Adjust offset for negative gregorian dates. */
            if(etm.tm_year <= -YEAR_BASE) ++etm.tm_year;
...
static void free_era_info(era_info_t *ei) {
    free(ei->era_C);
    free(ei->era_Y);
    free(ei);
}
...
#define ALT_STRLEN(a) strlen(a)
    ...
    adi->num = 1;
    for(a = alt_digits; (e = ALT_STRCHR(a, ';')) != NULL; a = e + 1) ++adi->num;
...
static void free_alt_digits(alt_digits_t *adi) {
    free(adi->digit);
    free(adi->buffer);
    free(adi);
}
...
static size_t __strftime(CHAR *s, size_t maxsize, const CHAR *format,
...
#endif /* !_WANT_C99_TIME_FORMATS */
{
    ...
    for(;;) {
        ...
        if(*format == CQ('0') || *format == CQ('+')) pad = *format++;
        ...
        switch(*format) {
            ...
            case CQ('c'):
                ...
                if(alt == 'E' && *era_info && *_CurrentTimeLocale->era_d_t_fmt)
                    ...
                    _ctloc(c_fmt);
                goto recurse;
            case CQ('r'): _ctloc(ampm_fmt); goto recurse;
            case CQ('x'):
#ifdef _WANT_C99_TIME_FORMATS
	    ...
	    */
...
#endif /* _WANT_C99_TIME_FORMATS */
                {
                    ...
                    {
                        fmt = CQ("%s%0.*d");
                        if(century >= 100 && pad == CQ('+')) pos = "+";
                    }
                    if(width < 2) width = 2;
                    len = snprintf(&s[count], maxsize - count, fmt, neg ? "-" : pos,
                                   width - neg, century);
                }
                CHECK_LENGTH();
            } break;
                ...
                if(alt == CQ('O') && *alt_digits) {
                    if(tim_p->tm_mday < 10) {
                        if(*format == CQ('d')) {
                            ...
                            len = conv_to_alt_digits(&s[count], maxsize - count, 0,
                                                     *alt_digits);
                            CHECK_LENGTH();
                        }
                        if(*format == CQ('e') || len == 0) s[count++] = CQ(' ');
                    }
                    len = conv_to_alt_digits(&s[count], maxsize - count, tim_p->tm_mday,
                                             *alt_digits);
                    CHECK_LENGTH();
                    if(len > 0) break;
                }
#endif /* _WANT_C99_TIME_FORMATS */
                len =
                    snprintf(&s[count], maxsize - count,
            ...
            case CQ('D'):
                /* %m/%d/%y */
                len =
                    snprintf(&s[count], maxsize - count, CQ("%.2d/%.2d/%.2d"),
                ...
                break;
            case CQ(
                'F'): { /* %F is equivalent to "%+4Y-%m-%d", flags and width can change
		 ...
		 the asymmetric range of years.  */
	       ...
	       of digits, we use 4 for consistency with 'Y'.  */
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt == CQ('O') && *alt_digits) {
                    len = conv_to_alt_digits(&s[count], maxsize - count, tim_p->tm_hour,
                                             *alt_digits);
                    CHECK_LENGTH();
                    if(len > 0) break;
                }
            ...
            case CQ('l'): /* newlib extension */
                if(alt == CQ('O')) alt = CQ('\0');
                /*FALLTHRU*/
            case CQ('I'): {
                register int h12;
                h12 = (tim_p->tm_hour == 0 || tim_p->tm_hour == 12) ? 12
                                                                    : tim_p->tm_hour % 12;
#ifdef _WANT_C99_TIME_FORMATS
                if(alt != CQ('O') || !*alt_digits ||
                   !(len = conv_to_alt_digits(&s[count], maxsize - count, h12,
                                              *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
            ...
            case CQ('j'):
                len =
                    snprintf(&s[count], maxsize - count, CQ("%.3d"), tim_p->tm_yday + 1);
                CHECK_LENGTH();
                break;
            case CQ('m'):
#ifdef _WANT_C99_TIME_FORMATS
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt != CQ('O') || !*alt_digits ||
                   !(len = conv_to_alt_digits(&s[count], maxsize - count, tim_p->tm_min,
                                              *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
                    len = snprintf(&s[count], maxsize - count, CQ("%.2d"), tim_p->tm_min);
                CHECK_LENGTH();
                break;
            case CQ('n'):
                if(count < maxsize - 1)
            ...
            case CQ('P'):
                ...
                for(i = 0; i < ctloclen; i++) {
                    if(count < maxsize - 1)
                        s[count++] = (*format == CQ('P') ? TOLOWER(ctloc[i]) : ctloc[i]);
                    else
                        return 0;
                }
                break;
 ...
 */
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt != CQ('O') || !*alt_digits ||
                   !(len = conv_to_alt_digits(&s[count], maxsize - count, tim_p->tm_sec,
                                              *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
                    len = snprintf(&s[count], maxsize - count, CQ("%.2d"), tim_p->tm_sec);
                CHECK_LENGTH();
                break;
            case CQ('t'):
                if(count < maxsize - 1)
		   ...
		   previous year was leap year.  */
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt != CQ('O') || !*alt_digits ||
                   !(len = conv_to_alt_digits(&s[count], maxsize - count, week,
                                              *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt == CQ('O') && *alt_digits) {
                    len = conv_to_alt_digits(&s[count], maxsize - count, tim_p->tm_wday,
                                             *alt_digits);
                    CHECK_LENGTH();
                    if(len > 0) break;
                }
...
#ifdef _WANT_C99_TIME_FORMATS
                if(alt != CQ('O') || !*alt_digits ||
                   !(len = conv_to_alt_digits(&s[count], maxsize - count, wday,
                                              *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
            ...
            case CQ('y'): {
                ...
                if(alt == 'E' && *era_info)
                    len =
                        snprintf(&s[count], maxsize - count, CQ("%d"), (*era_info)->year);
                else
#endif /* _WANT_C99_TIME_FORMATS */
                {
                    /* Be careful of both overflow and negative years, thanks to
...
#ifdef _WANT_C99_TIME_FORMATS
                    if(alt != CQ('O') || !*alt_digits ||
                       !(len = conv_to_alt_digits(&s[count], maxsize - count, year,
                                                  *alt_digits)))
#endif /* _WANT_C99_TIME_FORMATS */
            ...
            case CQ('Y'):
...
#endif /* _WANT_C99_TIME_FORMATS */
                {
                    ...
                    register unsigned year =
                        (unsigned)tim_p->tm_year + (unsigned)YEAR_BASE;
                    if(sign) {
                        *fmt++ = CQ('-');
...
#endif
                    if(!tznam) tznam = _tzname[tim_p->tm_isdst > 0];
...
size_t strftime(CHAR *__restrict s, size_t maxsize, const CHAR *__restrict format,
    ...
    size_t ret = __strftime(s, maxsize, format, tim_p, __get_current_locale(), &era_info,
                            &alt_digits);
    if(era_info) free_era_info(era_info);
    if(alt_digits) free_alt_digits(alt_digits);
    return ret;
#else  /* !_WANT_C99_TIME_FORMATS */
...
size_t strftime_l(CHAR *__restrict s, size_t maxsize, const CHAR *__restrict format,
...
const struct tm tmyr0 = {
    ...
    .tm_mday = 2, .tm_mon = 6,    .tm_year = INT_MAX - YEAR_BASE / 2,
...
char *_getenv_r(struct _reent *p, const char *cp) {
    return getenv(cp);
}
#endif
    ...
    for(l = 0; l < sizeof(List) / sizeof(List[0]); l++) {
        ...
        for(i = 0; i < test->cnt; i++) {
            ...
            if(ret != test->vec[i].ret) {
                errr++;
                fprintf(stderr, "ERROR:  return %d != %d expected for List[%d].vec[%d]\n",
