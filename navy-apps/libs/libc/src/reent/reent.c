...
void cleanup_glue(struct _reent *ptr, struct _glue *glue) {
    ...
    if(glue->_next) cleanup_glue(ptr, glue->_next);
...
void _reclaim_reent(struct _reent *ptr) {
    if(ptr != _impure_ptr) {
        ...
        if(ptr->_mp) /* don't bother allocating it! */
        {
            ...
            if(_REENT_MP_RESULT(ptr)) _free_r(ptr, _REENT_MP_RESULT(ptr));
#ifdef _REENT_SMALL
        }
#endif

#ifdef _REENT_SMALL
        if(ptr->_emergency) _free_r(ptr, ptr->_emergency);
        if(ptr->_mp) _free_r(ptr, ptr->_mp);
        if(ptr->_r48) _free_r(ptr, ptr->_r48);
        if(ptr->_localtime_buf) _free_r(ptr, ptr->_localtime_buf);
        if(ptr->_asctime_buf) _free_r(ptr, ptr->_asctime_buf);
        if(ptr->_signal_buf) _free_r(ptr, ptr->_signal_buf);
        if(ptr->_misc) _free_r(ptr, ptr->_misc);
#endif
        ...
        if(ptr->_cvtbuf) _free_r(ptr, ptr->_cvtbuf);
	...
	_free_r (ptr, ptr->_sig_func);*/
        ...
        if(ptr->__sdidinit) {
	     ...
	     before the program exits, and who wants to wait for that? */
            ...
            if(ptr->__sglue._next) cleanup_glue(ptr, ptr->__sglue._next);
        }
