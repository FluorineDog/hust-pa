...
static _READ_WRITE_RETURN_TYPE fmemreader(struct _reent *ptr, void *cookie, char *buf,
                                          _READ_WRITE_BUFSIZE_TYPE n) {
    fmemcookie *c = (fmemcookie *)cookie;
    /* Can't read beyond current size, but EOF condition is not an error.  */
    if(c->pos > c->eof) return 0;
    if(n >= c->eof - c->pos) n = c->eof - c->pos;
...
static _READ_WRITE_RETURN_TYPE fmemwriter(struct _reent *ptr, void *cookie,
    ...
    if(c->pos + n > c->eof) {
        c->eof = c->pos + n;
        if(c->eof - adjust < c->max) c->saved = c->buf[c->eof - adjust] = '\0';
    } else if(c->writeonly) {
        if(n) {
...
/* Seek to position POS relative to WHENCE within stream described by
   COOKIE; return resulting position or fail with EOF.  */
#ifdef __LARGE64_FILES
static _fpos64_t fmemseeker64(struct _reent *ptr, void *cookie, _fpos64_t pos,
                              int whence) {
    _off64_t offset = (_off64_t)pos;
    fmemcookie *c = (fmemcookie *)cookie;
    if(whence == SEEK_CUR)
...
FILE *_fmemopen_r(struct _reent *ptr, void *__restrict buf, size_t size,
    ...
    if((flags = __sflags(ptr, mode, &dummy)) == 0) return NULL;
    if(!size || !(buf || flags & __SRW)) {
        ptr->_errno = EINVAL;
        return NULL;
    }
    if((fp = __sfp(ptr)) == NULL) return NULL;
    if((c = (fmemcookie *)_malloc_r(ptr, sizeof *c + (buf ? 0 : size))) == NULL) {
