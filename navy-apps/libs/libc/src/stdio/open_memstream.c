...
static _READ_WRITE_RETURN_TYPE memwriter(struct _reent *ptr, void *cookie,
    ...
    if(c->pos + n >= c->max) {
        ...
        if(newsize < c->pos + n + 1) newsize = c->pos + n + 1;
        cbuf = _realloc_r(ptr, cbuf, newsize);
        if(!cbuf) return EOF; /* errno already set to ENOMEM */
        *c->pbuf = cbuf;
        c->max = newsize;
    }
    /* If we have previously done a seek beyond eof, ensure all
     intermediate bytes are NUL.  */
    if(c->pos > c->eof) memset(cbuf + c->eof, '\0', c->pos - c->eof);
...
/* Seek to position POS relative to WHENCE within stream described by
   COOKIE; return resulting position or fail with EOF.  */
#ifdef __LARGE64_FILES
static _fpos64_t memseeker64(struct _reent *ptr, void *cookie, _fpos64_t pos,
                             int whence) {
...
static int memcloser(struct _reent *ptr, void *cookie) {
    ...
    buf = _realloc_r(ptr, *c->pbuf,
                     c->wide > 0 ? (*c->psize + 1) * sizeof(wchar_t) : *c->psize + 1);
    if(buf) *c->pbuf = buf;
...
static FILE *internal_open_memstream_r(struct _reent *ptr, char **buf, size_t *size,
                                       int wide) {
    FILE *fp;
    memstream *c;
    ...
    c->max = *size;
    if(wide == 1) c->max *= sizeof(wchar_t);
    if(c->max < 64) c->max = 64;
#if(SIZE_MAX >= 64 * 1024)
