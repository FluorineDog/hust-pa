...
static const double q[] = {-0.21108770058106271242e+7, 0.36162723109421836460e+5,
                           -0.27773523119650701667e+3};
static const double p[] = {-0.35181283430177117881e+6, -0.11563521196851768270e+5,
                           -0.16375798202630751372e+3, -0.78966127417357099479};
...
double sineh(double x, int cosineh) {
    ...
    switch(numtest(x)) {
        ...
        case INF: errno = ERANGE; return (ispos(x) ? z_infinity.d : -z_infinity.d);
    }

    y = fabs(x);

    if(!cosineh && x < 0.0) sgn = -1;
    ...
    if((y > 1.0 && !cosineh) || cosineh) {
        if(y > BIGX) {
            ...
            if(w > WBAR) res = z * (V_OVER2_MINUS1 + 1.0);
        }
