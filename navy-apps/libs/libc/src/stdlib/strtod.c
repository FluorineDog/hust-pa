...
static void ULtod(__ULong *L, __ULong *bits, Long exp, int k) {
    switch(k & STRTOG_Retmask) {
        ...
        case STRTOG_Zero: L[0] = L[1] = 0; break;
        ...
        case STRTOG_NaN: L[_0] = 0x7fffffff; L[_1] = (__ULong)-1;
    }
    if(k & STRTOG_Neg) L[_0] |= 0x80000000L;
}
#endif /* !NO_HEX_FP */
...
double _strtod_l(struct _reent *ptr, const char *__restrict s00, char **__restrict se,
    ...
    int scale;
#endif
    int bb2, bb5, bbe, bd2, bd5, bbbits, bs2, c, decpt, dsign, e, e1, esign, i, j, k, nd,
        nd0, nf, nz, nz0, sign;
    const char *s, *s0, *s1;
    double aadj, adj;
    U aadj1, rv, rv0;
...
#define fpi1 fpi
#endif
                        switch((i = gethex(ptr, &s, &fpi1, &exp, &bb, sign, loc)) &
                               STRTOG_Retmask) {
                            case STRTOG_NoNumber:
                                s = s00;
                                sign = 0;
                                /* FALLTHROUGH */
                            case STRTOG_Zero: break;
                            default:
                                if(bb) {
       ...
       && Flt_Rounds == 1
        ...
        if(e > 0) {
            ...
            vax_ovfl_check:
                ...
                /* rv = */ rounded_product(dval(rv), tens[e]);
                if((dword0(rv) & Exp_mask) > Exp_msk1 * (DBL_MAX_EXP + Bias - 1 - P))
                    goto ovfl;
                dword0(rv) += P * Exp_msk1;
#else
                /* rv = */ rounded_product(dval(rv), tens[e]);
    ...
    if(e1 > 0) {
        ...
        if(e1 &= ~15) {
            ...
            ovfl:
                ...
                switch(rounding) {
                    case 0: /* toward 0 */
                    case 3: /* toward -infinity */ dword0(rv) = Big0;
#ifndef _DOUBLE_IS_32BITS
                        dword1(rv) = Big1;
#endif /*!_DOUBLE_IS_32BITS*/
                        break;
                    default: dword0(rv) = Exp_mask;
#ifndef _DOUBLE_IS_32BITS
                        dword1(rv) = 0;
#endif /*!_DOUBLE_IS_32BITS*/
                }
            ...
            dword0(rv) -= P * Exp_msk1;
            dval(rv) *= bigtens[j];
            if((z = dword0(rv) & Exp_mask) > Exp_msk1 * (DBL_MAX_EXP + Bias - P))
                goto ovfl;
        ...
        if(e1 >>= 4) {
            ...
            if(scale && (j = 2 * P + 1 - ((dword0(rv) & Exp_mask) >> Exp_shift)) > 0) {
...
if(bd0 == NULL) goto ovfl;

for(;;) {
    bd = Balloc(ptr, bd0->_k);
    if(bd == NULL) goto ovfl;
    Bcopy(bd, bd0);
    bb = d2b(ptr, dval(rv), &bbe, &bbbits); /* rv = bb * 2^bbe */
    if(bb == NULL) goto ovfl;
    bs = i2b(ptr, 1);
    if(bs == NULL) goto ovfl;
    ...
    if(rounding != 1) {
        if(i < 0) {
            ...
            if(rounding) {
                ...
                adj = -1.;
                if(!dword1(rv) && !(dword0(rv) & Frac_mask)) {
        ...
        adj *= ulp(dval(rv));
        if(dsign) {
            if(dword0(rv) == Big0 && dword1(rv) == Big1) goto ovfl;
            dval(rv) += adj;
            else dval(rv) -= adj;
            goto cont;
        }
...
#ifdef SET_INEXACT
                if(!delta->x[0] && delta->wds <= 1) inexact = 0;
#endif
                break;
            }
            if(!delta->_x[0] && delta->_wds <= 1) {
            ...
            if(dsign) {
                if((dword0(rv) & Bndry_mask1) == Bndry_mask1 &&
                   dword1(rv) ==
                    ...
                    if(dword0(rv) == Big0 && dword1(rv) == Big1) goto ovfl;
                    dword0(rv) = (dword0(rv) & Exp_mask) + Exp_msk1
#ifdef IBM
                                 | Exp_msk1 >> 4
#endif
                        ;
...
#ifdef Avoid_Underflow
            if(Lsb1) {
                if(!(dword0(rv) & Lsb1)) break;
            } else if(!(dword1(rv) & Lsb))
                break;
#else
            if(!(dword1(rv) & LSB)) break;
#endif
#endif
            if(dsign)
#ifdef Avoid_Underflow
        ...
        if((aadj = ratio(delta, bs)) <= 2.) {
...
#ifndef Sudden_Underflow
                if(dword1(rv) == Tiny1 && !dword0(rv)) goto undfl;
#endif
                aadj = 1.;
                dval(aadj1) = -1.;
            } else {
            ...
            switch(Rounding) {
                case 2: /* towards +infinity */ dval(aadj1) -= 0.5; break;
                case 0: /* towards 0 */
                case 3: /* towards -infinity */ dval(aadj1) += 0.5;
            }
#else
                if(Flt_Rounds == 0) dval(aadj1) += 0.5;
        ...
        if(y == Exp_msk1 * (DBL_MAX_EXP + Bias - 1)) {
            ...
            dval(rv) += adj;
            if((dword0(rv) & Exp_mask) >= Exp_msk1 * (DBL_MAX_EXP + Bias - P)) {
                if(dword0(rv0) == Big0 && dword1(rv0) == Big1) goto ovfl;
                dword0(rv) = Big0;
#ifndef _DOUBLE_IS_32BITS
                dword1(rv) = Big1;
#endif /*!_DOUBLE_IS_32BITS*/
...
#endif
                    {
                        if(dword0(rv0) == Tiny0 && dword1(rv0) == Tiny1) goto undfl;
#ifndef _DOUBLE_IS_32BITS
                        dword0(rv) = Tiny0;
                        dword1(rv) = Tiny1;
#else
			 ...
			 */
                if(y <= (P - 1) * Exp_msk1 && aadj > 1.) {
                    ...
                    if(!dsign) dval(aadj1) = -dval(aadj1);
                }
                adj = dval(aadj1) * ulp(dval(rv));
                dval(rv) += adj;
#endif /*Sudden_Underflow*/
    ...
    if(scale) {
        ...
        if(dword0(rv) == 0 && dword1(rv) == 0) ptr->_errno = ERANGE;
#endif
    }
#endif /* Avoid_Underflow */
#ifdef SET_INEXACT
...
double _strtod_r(struct _reent *ptr, const char *__restrict s00, char **__restrict se) {
    return _strtod_l(ptr, s00, se, __get_current_locale());
}

#ifndef _REENT_ONLY

double strtod_l(const char *__restrict s00, char **__restrict se, locale_t loc) {
...
float strtof_l(const char *__restrict s00, char **__restrict se, locale_t loc) {
    double val = _strtod_l(_REENT, s00, se, loc);
    if(isnan(val)) return signbit(val) ? -nanf("") : nanf("");
