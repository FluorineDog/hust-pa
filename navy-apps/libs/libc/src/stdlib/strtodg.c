...
static int all_on(_Bigint *b, int n) {
    ...
    xe = x + (n >> kshift);
    while(x < xe)
        if((*x++ & ALL_ON) != ALL_ON) return 0;
    if(n &= kmask) return ((*x | (ALL_ON << n)) & ALL_ON) == ALL_ON;
    return 1;
}
...
static int rvOK(struct _reent *p, double d, FPI *fpi, Long *exp, __ULong *bits, int exact,
                int rd, int *irv) {
    _Bigint *b;
    ...
    switch(rd) {
        ...
        default: /* round near */
            ...
            if(b->_x[k >> kshift] & ((__ULong)1 << (k & kmask))) break;
            goto trunc;
    }
...
trunc:
    inex = lostbits = 0;
    if(bdif > 0) {
        if((lostbits = any_on(b, bdif)) != 0) inex = STRTOG_Inexlo;
        rshift(b, bdif);
        if(carry) {
            inex = STRTOG_Inexhi;
            b = increment(p, b);
            if((j = nb & kmask) != 0) j = ULbits - j;
    ...
    if(e < fpi->emin) {
        ...
        if(k > nb || fpi->sudden_underflow) {
            ...
            *irv = STRTOG_Underflow | STRTOG_Inexlo;
            ...
            k1 = k - 1;
            if(k1 > 0 && !lostbits) lostbits = any_on(b, k1);
            if(!lostbits && !exact) goto ret;
            lostbits |= carry = b->_x[k1 >> kshift] & (1 << (k1 & kmask));
            rshift(b, k);
            *irv = STRTOG_Denormal;
            if(carry) {
                b = increment(p, b);
...
int _strtodg_l(struct _reent *p, const char *s00, char **se, FPI *fpi, Long *exp,
...
dig_done:
    ...
    dval(rv) = y;
    if(k > 9) dval(rv) = tens[k - 9] * dval(rv) + z;
    bd0 = 0;
    if(nbits <= P && nd <= DBL_DIG) {
        if(!e) {
            if(rvOK(p, dval(rv), fpi, exp, bits, 1, rd, &irv)) goto ret;
        } else if(e > 0) {
            if(e <= Ten_pmax) {
#ifdef VAX
                goto vax_ovfl_check;
                ...
                if(rvOK(p, dval(rv), fpi, exp, bits, i, rd, &irv)) goto ret;
                e1 -= e;
                goto rv_notOK;
#endif
            }
...
#endif
                if(rvOK(p, dval(rv), fpi, exp, bits, 0, rd, &irv)) goto ret;
                e1 -= e2;
            }
        }
...
rv_notOK:
    ...
    for(;;) {
        ...
        if(bb2 > 0)
            ...
            rshift(bb, -bb2);
        if(bd5 > 0) bd = pow5mult(p, bd, bd5);
        if(bd2 > 0) bd = lshift(p, bd, bd2);
        if(bs2 > 0) bs = lshift(p, bs, bs2);
        asub = 1;
        inex = STRTOG_Inexhi;
        delta = diff(p, bb, bd);
        ...
        if(rd && i <= 0) {
            ...
            if((finished = dsign ^ (rd & 1)) != 0) {
                ...
                for(i = 0, j = nbits; j >= ULbits; i++, j -= ULbits) {
                    if(rvb->_x[i] & ALL_ON) goto adj1;
                }
                if(j > 1 && lo0bits(rvb->_x + i) < j - 1) goto adj1;
                rve = rve1 - 1;
        ...
        if(i == 0) {
            ...
            if(dsign) {
                ...
                if(hi0bits(rvb->_x[rvb->_wds - 1]) != j) rvbits++;
                irv = STRTOG_Normal | STRTOG_Inexhi;
            } else {
        ...
        if(asub) {
            ...
            if(denorm)
                /* do nothing */;
            else if(rvb->_wds <= k || hi0bits(rvb->_x[k]) > hi0bits(rvb0->_x[k])) {
            ...
            rvb = sum(p, rvb, ab);
            k = rvb->_wds - 1;
            if(k >= rvb0->_wds || hi0bits(rvb->_x[k]) < hi0bits(rvb0->_x[k])) {
                if(denorm) {
...
ret:
    if(denorm) {
        if(sudden_underflow) {
            ...
            irv =
                (irv & ~STRTOG_Retmask) | (rvb->_wds > 0 ? STRTOG_Denormal : STRTOG_Zero);
