...
static void etoasc(short unsigned int *x, char *string, int ndigs, int outformat,
                   LDPARMS *ldp);
...
static int eisnan(const short unsigned int *x) {
    ...
    if((x[NE - 1] & 0x7fff) != 0x7fff) return (0);
...
#ifdef USE_INFINITY
    for(i = 0; i < NE - 1; i++) *x++ = 0;
    *x |= 32767;
    ldp = ldp;
#else
    for(i = 0; i < NE - 1; i++) *x++ = 0xffff;
    *x |= 32766;
    if(ldp->rndprc < NBITS) {
        if(ldp->rndprc == 113) {
            *(x - 9) = 0;
...
#ifdef USE_INFINITY
        s[1] = 32767;
        for(i = 2; i < NI - 1; i++) s[i] = 0;
#else
        s[1] = 32766;
        s[2] = 0;
        for(i = M + 1; i < NI - 1; i++) s[i] = 0xffff;
        s[NI - 1] = 0;
...
#ifdef NANS
#ifdef IBMPC
        if(((pe[3] & 0xf) != 0) || (pe[2] != 0) || (pe[1] != 0) || (pe[0] != 0)) {
            enan(y, NBITS);
            return;
        }
#else  /* !IBMPC */
        if(((pe[0] & 0xf) != 0) || (pe[1] != 0) || (pe[2] != 0) || (pe[3] != 0)) {
            enan(y, NBITS);
            return;
        }
#endif /* !IBMPC */
...
#endif /* !IBMPC */
    (void)eshift(yy, -5);
    if(denorm) { /* if zero exponent, then normalize the significand */
...
static void etoe53(x, e) unsigned short *x, *e;
{ etodec(x, e); /* see etodec.c */ }

static void __attribute__((__unused__)) toe53(x, y) unsigned short *x, *y;
{ todec(x, y); }
...
toe53(short unsigned int *x, short unsigned int *y) {
    ...
    *y = 0;               /* output high order */
    if(*p++) *y = 0x8000; /* output sign bit */

    i = *p++;
    if(i >= (unsigned int)2047) { /* Saturate at largest number less than infinity. */
#ifdef USE_INFINITY
        *y |= 0x7ff0;
#ifdef IBMPC
        *(--y) = 0;
...
toe24(short unsigned int *x, short unsigned int *y) {
    ...
    *y = 0;               /* output high order */
    ...
    i = *p++;
    if(i >= 255) { /* Saturate at largest number less than infinity. */
#ifdef USE_INFINITY
...
static int enormlz(short unsigned int *x) {
    ...
    if(*p != 0) goto normdn;
    ++p;
    if(*p & 0x8000) return (0); /* already normalized */
    while(*p == 0) {
        eshup6(x);
        sc += 16;
...
char *_ldtoa_r(struct _reent *ptr, long double d, int mode, int ndigits, int *decpt,
               int *sign, char **rve) {
    unsigned short e[NI];
    char *s, *p;
    int i, j, k;
...
static void etoasc(short unsigned int *x, char *string, int ndigits, int outformat,
                   LDPARMS *ldp) {
    long digit;
    ...
    if(y[NE - 1] == 0) {
        for(k = 0; k < NE - 1; k++) {
            if(y[k] != 0) goto tnzro; /* denormalized number */
        }
...
*/
    ...
    if(digit > 4) {
        ...
        if(digit == 5) {
            emovo(y, t, ldp);
            if(ecmp(t, ezero) != 0) goto roun; /* round to nearest */
...
#ifdef MIEEE
#if !defined(__mips)
static const unsigned short nan113[8] = {0x7fff, 0xffff, 0xffff, 0xffff,
                                         0xffff, 0xffff, 0xffff, 0xffff};

static const unsigned short nan64[6] = {0x7fff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff};
...
static const unsigned short nan113[8] = {0x7fff, 0x7fff, 0xffff, 0xffff,
                                         0xffff, 0xffff, 0xffff, 0xffff};

static const unsigned short nan64[6] = {0x7fff, 0xbfff, 0xffff, 0xffff, 0xffff, 0xffff};
...
#else  /* __mips && !__mips_nan2008 */
static const unsigned short nan113[8] = {0xffff, 0xffff, 0xffff, 0xffff,
                                         0xffff, 0xffff, 0x7fff, 0x7fff};

static const unsigned short nan64[6] = {0xffff, 0xffff, 0xffff, 0xffff, 0xbfff, 0x7fff};
...
static void enan(short unsigned int *nan, int size) {
    ...
    switch(size) {
        ...
        case NBITS:
#if !defined(__mips) || defined(__mips_nan2008)
            for(i = 0; i < NE - 2; i++) *nan++ = 0;
            *nan++ = 0xc000;
#else  /* __mips && !__mips_nan2008 */
            for(i = 0; i < NE - 2; i++) *nan++ = 0xffff;
            *nan++ = 0xbfff;
#endif /* __mips && !__mips_nan2008 */
            *nan++ = 0x7fff;
            return;
...
#if !defined(__mips) || defined(__mips_nan2008)
            *nan++ = 0xc000;
            for(i = 4; i < NI - 1; i++) *nan++ = 0;
#else  /* __mips && !__mips_nan2008 */
            *nan++ = 0xbfff;
            for(i = 4; i < NI - 1; i++) *nan++ = 0xffff;
#endif /* __mips && !__mips_nan2008 */
            *nan++ = 0;
            return;
#endif
        default: mtherr("enan", DOMAIN); return;
    }
    for(i = 0; i < n; i++) *nan++ = *p++;
}
