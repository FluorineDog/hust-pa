...
char *_dtoa_r(struct _reent *ptr, double _d, int mode, int ndigits, int *decpt, int *sign,
              char **rve) {
	...
	*/
    ...
    word0(d2) |= Exp_11;
#ifdef IBM
    if(j = 11 - hi0bits(word0(d2) & Frac_mask)) d2.d /= 1 << j;
#endif
...
if(b5 > 0) {
    if(leftright) {
        ...
        if((j = b5 - m5) != 0) b = pow5mult(ptr, b, j);
    } else
        b = pow5mult(ptr, b, b5);
}
S = i2b(ptr, 1);
if(s5 > 0) S = pow5mult(ptr, S, s5);
...
#ifdef Pack_32
if((i = ((s5 ? 32 - hi0bits(S->_x[S->_wds - 1]) : 1) + s2) & 0x1f) != 0) i = 32 - i;
#else
...
if(b2 > 0) b = lshift(ptr, b, b2);
if(s2 > 0) S = lshift(ptr, S, s2);
if(k_check) {
    if(cmp(b, S) < 0) {
        k--;
        b = multadd(ptr, b, 10, 0); /* we botched the k estimate */
        if(leftright) mhi = multadd(ptr, mhi, 10, 0);
        ilim = ilim1;
    }
}
if(ilim <= 0 && mode > 2) {
