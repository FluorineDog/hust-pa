...
static int getopt_internal(int argc, char *const argv[], const char *shortopts,
   ...
   */
    if(data->optwhere == 1) {
        switch(ordering) {
            ...
            case RETURN_IN_ORDER:
                ...
                break;
            case REQUIRE_ORDER:
                if(!is_option(argv[data->optind], only)) return EOF;
                break;
        }
    }
    /* End of option list? */
    if(argv[data->optind] == 0) return EOF;
    ...
    if(longopts != 0 &&
       ...
       data->optwhere == 1) {
        /* handle long options */
        if(memcmp(argv[data->optind], "--", 2) == 0) data->optwhere = 2;
        longopt_match = -1;
        ...
        for(optindex = 0; longopts[optindex].name != 0; ++optindex) {
            if(memcmp(argv[data->optind] + data->optwhere, longopts[optindex].name,
                      match_chars) == 0) {
    ...
    if(longopt_match < 0 && shortopts != 0) {
        ...
        has_arg =
            ((cp[1] == ':') ? ((cp[2] == ':') ? OPTIONAL_ARG : REQUIRED_ARG) : NO_ARG);
    ...
    switch(has_arg) {
        ...
        case NO_ARG:
            if(longopt_match < 0) {
                data->optwhere++;
                if(argv[data->optind][data->optwhere] == '\0') data->optwhere = 1;
            } else
                data->optwhere = 1;
            data->optarg = 0;
            break;
    ...
    /* finally return */
    if(longopt_match >= 0) {
        if(longind != 0) *longind = longopt_match;
