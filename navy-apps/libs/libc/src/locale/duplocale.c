...
struct __locale_t *_duplocale_r(struct _reent *p, struct __locale_t *locobj) {
    ...
    if(locobj == LC_GLOBAL_LOCALE) locobj = __get_global_locale();
    ...
    for(i = 1; i < _LC_LAST; ++i)
	   ...
	   __loadlocale.  It knows what to do to replicate the category. */
            ...
            if(!__loadlocale(&tmp_locale, i, locobj->categories[i])) goto error;
        }
