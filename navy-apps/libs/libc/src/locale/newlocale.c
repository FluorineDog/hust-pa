...
#define LC_VALID_MASK                                                       \
    (LC_COLLATE_MASK | LC_CTYPE_MASK | LC_MONETARY_MASK | LC_NUMERIC_MASK | \
     LC_TIME_MASK | LC_MESSAGES_MASK)

struct __locale_t *_newlocale_r(struct _reent *p, int category_mask, const char *locale,
    ...
    for(i = 1; i < _LC_LAST; ++i) {
        if(((1 << i) & category_mask) != 0) {
	     ...
	     name verbatim. */
            const char *cat = (locale[0] == '\0') ? __get_locale_env(p, i) : locale;
    ...
    for(i = 1; i < _LC_LAST; ++i) {
	 ...
	 or the new category is the base categroy, just copy over. */
        ...
        /* Otherwise, if the category is in category_mask, create entry. */
            ...
            if(!strcmp(new_categories[i], "C") || !strcmp(new_categories[i], "POSIX"))
                continue;
...
#ifdef __HAVE_LOCALE_INFO__
    for(i = 1; i < _LC_LAST; ++i)
        if(((1 << i) & category_mask) != 0 && tmp_locale.lc_cat[i].buf &&
