...
int wordexp(const char *__restrict words, wordexp_t *__restrict pwordexp, int flags) {
    ...
    if(flags & WRDE_DOOFFS) {
        ...
        if(pwordexp->we_wordv) wordv = WE_WORDV_TO_EXT_WORDV(pwordexp->we_wordv);
        wordv = (ext_wordv_t *)realloc(
            wordv, sizeof(ext_wordv_t) + (offs + pwordexp->we_wordc) * sizeof(char *));
        if(!wordv) return err;
        if(!pwordexp->we_wordv) SLIST_INIT(&wordv->list);
        pwordexp->we_wordv = wordv->we_wordv;

        for(i = 0; i < offs; i++) pwordexp->we_wordv[i] = NULL;
    }

    if(pipe(fd)) return err;
    if(pipe(fd_err)) {
        close(fd[0]);
        close(fd[1]);
        return err;
    ...
    if(pid == -1) {
        ...
        if(!(f_err = fdopen(fd_err[0], "r"))) goto cleanup;
        ...
        if(fgets(tmp, MAXLINELEN, f_err)) {
            if(strstr(tmp, "EOF"))
                err = WRDE_SYNTAX;
                    ...
                    strstr(tmp, "`;'") || strstr(tmp, "`<'") || strstr(tmp, "`>'") ||
                    strstr(tmp, "`('") || strstr(tmp, "`)'") || strstr(tmp, "`{'") ||
                    strstr(tmp, "`}'"))
                err = WRDE_BADCHAR;
            ...
            if(flags & WRDE_SHOWERR) {
                fputs(tmp, stderr);
                while(fgets(tmp, MAXLINELEN, f_err)) fputs(tmp, stderr);
            }
        ...
        if(!(f = fdopen(fd[0], "r"))) goto cleanup;

        /* Get number of words expanded by shell. */
        if(!fgets(tmp, MAXLINELEN, f)) goto cleanup;
        ...
        if(pwordexp->we_wordv) wordv = WE_WORDV_TO_EXT_WORDV(pwordexp->we_wordv);
        wordv = (ext_wordv_t *)realloc(
            wordv, sizeof(ext_wordv_t) +
                       (offs + pwordexp->we_wordc + num_words) * sizeof(char *));
        if(!wordv) return err;
        if(!pwordexp->we_wordv) SLIST_INIT(&wordv->list);
        pwordexp->we_wordv = wordv->we_wordv;
        ...
        if(!(entry = (struct ewords_entry *)malloc(sizeof(struct ewords_entry) +
                                                   num_bytes + num_words)))
            goto cleanup;
        SLIST_INSERT_HEAD(&wordv->list, entry, next);

        /* Get expansion from the shell output. */
        if(!fread(entry->ewords, 1, num_bytes + num_words, f)) goto cleanup;
        ...
        if(i == num_words) err = WRDE_SUCCESS;
    ...
    cleanup:
        ...
        if(fd[1] != STDOUT_FILENO) {
            if(dup2(fd[1], STDOUT_FILENO) == -1) _exit(EXIT_FAILURE);
        ...
        if(fd_err[1] != STDERR_FILENO) {
            if(dup2(fd_err[1], STDERR_FILENO) == -1) _exit(EXIT_FAILURE);
