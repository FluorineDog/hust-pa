...
void luaV_finishget(lua_State *L, const TValue *t, TValue *key, StkId val,
    ...
    for(loop = 0; loop < MAXTAGLOOP; loop++) {
        if(slot == NULL) { /* 't' is not a table? */
            ...
            if(ttisnil(tm)) luaG_typeerror(L, t, "index"); /* no metamethod */
...
*/
void luaV_finishset(lua_State *L, const TValue *t, TValue *key, StkId val,
                    const TValue *slot) {
    int loop; /* counter to avoid infinite loops */
    for(loop = 0; loop < MAXTAGLOOP; loop++) {
        ...
        if(luaV_fastset(L, t, key, slot, luaH_get, val)) return; /* done */
                                                                 /* else loop */
    }
    ...
    for(;;) {
        ...
        vmdispatch(GET_OPCODE(i)) {
            ...
            vmcase(OP_EQ) {
                TValue *rb = RKB(i);
                TValue *rc = RKC(i);
                Protect(if(luaV_equalobj(L, rb, rc) != GETARG_A(i)) ci->u.l.savedpc++;
                        else donextjump(ci);) vmbreak;
            }
            vmcase(OP_LT) {
            ...
            vmcase(OP_VARARG) {
                ...
                for(j = 0; j < b && j < n; j++) setobjs2s(L, ra + j, base - n + j);
