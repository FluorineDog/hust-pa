...
static void fixjump(FuncState *fs, int pc, int dest) {
    ...
    if(abs(offset) > MAXARG_sBx) luaX_syntaxerror(fs->ls, "control structure too long");
...
void luaK_patchclose(FuncState *fs, int list, int level) {
    ...
    for(; list != NO_JUMP; list = getjump(fs, list)) {
        lua_assert(
            GET_OPCODE(fs->f->code[list]) == OP_JMP &&
            (GETARG_A(fs->f->code[list]) == 0 || GETARG_A(fs->f->code[list]) >= level));
...
static int luaK_code(FuncState *fs, Instruction i) {
    ...
    luaM_growvector(fs->ls->L, f->lineinfo, fs->pc, f->sizelineinfo, int, MAX_INT,
                    "opcodes");
...
static void codecomp(FuncState *fs, BinOpr opr, expdesc *e1, expdesc *e2) {
    int rk1 =
        (e1->k == VK) ? RKASK(e1->u.info) : check_exp(e1->k == VNONRELOC, e1->u.info);
...
void luaK_prefix(FuncState *fs, UnOpr op, expdesc *e, int line) {
    ...
    switch(op) {
        ...
        case OPR_BNOT: /* use 'ef' as fake 2nd operand */
            if(constfolding(fs, op + LUA_OPUNM, e, &ef)) break;
            /* FALLTHROUGH */
        case OPR_LEN: codeunexpval(fs, cast(OpCode, op + OP_UNM), e, line); break;
...
void luaK_posfix(FuncState *fs, BinOpr op, expdesc *e1, expdesc *e2, int line) {
    switch(op) {
        ...
        case OPR_CONCAT: {
            luaK_exp2val(fs, e2);
            if(e2->k == VRELOCABLE && GET_OPCODE(getinstruction(fs, e2)) == OP_CONCAT) {
