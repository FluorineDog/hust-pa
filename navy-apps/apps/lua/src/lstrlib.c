    ...
    switch(*p++) {
        ...
        case '[': {
            ...
            do { /* look for a ']' */
                ...
                if(*(p++) == L_ESC && p < ms->p_end) p++; /* skip escapes (e.g. '%]') */
            } while(*p != ']');
            return p + 1;
        }
        default: { return p; }
...
init:                    /* using goto's to optimize tail recursion */
    if(p != ms->p_end) { /* end of pattern? */
        switch(*p) {
            ...
            case ')': { /* end capture */ s = end_capture(ms, s, p + 1); break;
            }
            case '$': {
...
static int lua_number2strx(lua_State *L, char *buff, int sz, const char *fmt,
                           lua_Number x) {
...
static const char *scanformat(lua_State *L, const char *strfrmt, char *form) {
    ...
    if(isdigit(uchar(*p))) luaL_error(L, "invalid format (width or precision too long)");
    *(form++) = '%';
...
static KOption getoption(Header *h, const char **fmt, int *size) {
    ...
    *size = 0; /* default */
    switch(opt) {
        ...
        case 'c':
            *size = getnum(fmt, -1);
            if(*size == -1) luaL_error(h->L, "missing size for format option 'c'");
            return Kchar;
        case 'z': return Kzstr;
...
** despite its size.
*/
static KOption getdetails(Header *h, size_t totalsize, const char **fmt, int *psize,
                          int *ntoalign) {
...
*/
static void copywithendian(volatile char *dest, volatile const char *src, int size,
                           int islittle) {
    ...
    while(*fmt != '\0') {
        ...
        switch(opt) {
            ...
            case Kstring: { /* strings with length count */
                ...
                const char *s = luaL_checklstring(L, arg, &len);
                luaL_argcheck(
                    L, size >= (int)sizeof(size_t) || len < ((size_t)1 << (size * NB)),
...
** overflow.
*/
static lua_Integer unpackint(lua_State *L, const char *str, int islittle, int size,
                             int issigned) {
    lua_Unsigned res = 0;
    int i;
...
static const luaL_Reg strlib[] = {
    ...
    {"find", str_find},         {"format", str_format}, {"gmatch", gmatch},
    {"gsub", str_gsub},         {"len", str_len},       {"lower", str_lower},
    {"match", str_match},       {"rep", str_rep},       {"reverse", str_reverse},
    {"sub", str_sub},           {"upper", str_upper},   {"pack", str_pack},
