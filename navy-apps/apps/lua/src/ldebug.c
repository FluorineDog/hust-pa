...
static int auxgetinfo(lua_State *L, const char *what, lua_Debug *ar, Closure *f,
    ...
    for(; *what; what++) {
        switch(*what) {
            ...
            case 'L':
            case 'f': /* handled by lua_getinfo */ break;
...
LUA_API int lua_getinfo(lua_State *L, const char *what, lua_Debug *ar) {
    ...
    if(strchr(what, 'L')) collectvalidlines(L, cl);
    lua_unlock(L);
    return status;
}
...
static const char *funcnamefromcode(lua_State *L, CallInfo *ci, const char **name) {
    ...
    switch(GET_OPCODE(i)) {
        ...
        default: return NULL; /* cannot find a reasonable name */
    }
...
static const char *varinfo(lua_State *L, const TValue *o) {
    ...
    if(isLua(ci)) {
        ...
        if(!kind && isinstack(ci, o))      /* no? try a register */
            kind = getobjname(ci_func(ci)->p, currentpc(ci), cast_int(o - ci->u.l.base),
                              &name);
    }
...
void luaG_traceexec(lua_State *L) {
    ...
    if(counthook) luaD_hook(L, LUA_HOOKCOUNT, -1); /* call count hook */
