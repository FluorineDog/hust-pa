...
#define l_popen(L, c, m) \
    ((void)((void)c, m), luaL_error(L, "'popen' not supported"), (FILE *)0)
...
#elif defined(LUA_USE_WINDOWS) && !defined(_CRTIMP_TYPEINFO) && defined(_MSC_VER) && \
    (_MSC_VER >= 1400) /* }{ */
...
static FILE *tofile(lua_State *L) {
    LStream *p = tolstream(L);
    if(isclosed(p)) luaL_error(L, "attempt to use a closed file");
    lua_assert(p->f);
    return p->f;
}
...
static void opencheck(lua_State *L, const char *fname, const char *mode) {
    ...
    if(p->f == NULL) luaL_error(L, "cannot open file '%s' (%s)", fname, strerror(errno));
}
...
static FILE *getiofile(lua_State *L, const char *findex) {
    ...
    if(isclosed(p)) luaL_error(L, "standard %s file is closed", findex + IOPREF_LEN);
    return p->f;
}
        ...
        if(lua_type(L, arg) == LUA_TNUMBER) {
            ...
            int len =
                lua_isinteger(L, arg)
                    ...
                    : fprintf(f, LUA_NUMBER_FMT, (LUAI_UACNUMBER)lua_tonumber(L, arg));
            status = status && (len > 0);
        } else {
            size_t l;
...
static const luaL_Reg flib[] = {{"close", io_close},        {"flush", f_flush},
                                ...
                                {"__tostring", f_tostring}, {NULL, NULL}};
