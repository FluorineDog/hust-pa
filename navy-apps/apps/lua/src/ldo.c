...
static void correctstack(lua_State *L, TValue *oldstack) {
    ...
    for(ci = L->ci; ci != NULL; ci = ci->previous) {
        ...
        if(isLua(ci)) ci->u.l.base = (ci->u.l.base - oldstack) + L->stack;
    }
}
...
void luaD_reallocstack(lua_State *L, int newsize) {
    ...
    for(; lim < newsize; lim++) setnilvalue(L->stack + lim); /* erase new segment */
    L->stacksize = newsize;
...
static StkId adjust_varargs(lua_State *L, Proto *p, int actual) {
    ...
    for(; i < nfixargs; i++) setnilvalue(L->top++); /* complete missing arguments */
    return base;
}
...
** separated.
*/
static int moveresults(lua_State *L, const TValue *firstResult, StkId res, int nres,
                       int wanted) {
...
int luaD_precall(lua_State *L, StkId func, int nresults) {
    ...
    CallInfo *ci;
    switch(ttype(func)) {
        case LUA_TCCL: /* C closure */ f = clCvalue(func)->f; goto Cfunc;
...
LUA_API int lua_resume(lua_State *L, lua_State *from, int nargs) {
    ...
    if(L->nCcalls >= LUAI_MAXCCALLS) return resume_error(L, "C stack overflow", nargs);
