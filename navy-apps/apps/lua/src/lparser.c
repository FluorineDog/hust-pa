...
static int registerlocalvar(LexState *ls, TString *varname) {
    ...
    luaM_growvector(ls->L, f->locvars, fs->nlocvars, f->sizelocvars, LocVar, SHRT_MAX,
                    "local variables");
...
static void new_localvar(LexState *ls, TString *name) {
    ...
    luaM_growvector(ls->L, dyd->actvar.arr, dyd->actvar.n + 1, dyd->actvar.size, Vardesc,
...
static void closegoto(LexState *ls, int g, Labeldesc *label) {
    ...
    if(gt->nactvar < label->nactvar) {
        ...
        const char *msg = luaO_pushfstring(
            ls->L, "<goto %s> at line %d jumps into the scope of local '%s'",
            getstr(gt->name), gt->line, getstr(vname));
        semerror(ls, msg);
    }
    luaK_patchlist(fs, gt->pc, label->pc);
    /* remove goto from pending list */
    for(i = g; i < gl->n - 1; i++) gl->arr[i] = gl->arr[i + 1];
    gl->n--;
}
...
static void assignment(LexState *ls, struct LHS_assign *lh, int nvars) {
    ...
    if(testnext(ls, ',')) { /* assignment -> ',' suffixedexp assignment */
        ...
        nv.prev = lh;
        suffixedexp(ls, &nv.v);
        if(nv.v.k != VINDEXED) check_conflict(ls, lh, &nv.v);
...
static void checkrepeated(FuncState *fs, Labellist *ll, TString *label) {
    ...
    for(i = fs->bl->firstlabel; i < ll->n; i++) {
        if(eqstr(label, ll->arr[i].name)) {
            const char *msg =
                luaO_pushfstring(fs->ls->L, "label '%s' already defined on line %d",
...
static void statement(LexState *ls) {
    ...
    switch(ls->t.token) {
        ...
        case TK_GOTO: { /* stat -> 'goto' NAME */ gotostat(ls, luaK_jump(ls->fs)); break;
        }
