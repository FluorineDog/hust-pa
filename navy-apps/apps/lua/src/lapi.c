...
LUA_API lua_Number lua_tonumberx(lua_State *L, int idx, int *pisnum) {
    ...
    if(!isnum) n = 0; /* call to 'tonumber' may change 'n' even if it fails */
...
LUA_API lua_Integer lua_tointegerx(lua_State *L, int idx, int *pisnum) {
    ...
    if(!isnum) res = 0; /* call to 'tointeger' may change 'n' even if it fails */
    if(pisnum) *pisnum = isnum;
    return res;
}
...
LUA_API void lua_createtable(lua_State *L, int narray, int nrec) {
    ...
    sethvalue(L, L->top, t);
    api_incr_top(L);
    if(narray > 0 || nrec > 0) luaH_resize(L, t, narray, nrec);
    luaC_checkGC(L);
    lua_unlock(L);
}
...
LUA_API int lua_getmetatable(lua_State *L, int objindex) {
    ...
    switch(ttnov(obj)) {
        case LUA_TTABLE: mt = hvalue(obj)->metatable; break;
        case LUA_TUSERDATA: mt = uvalue(obj)->metatable; break;
        default: mt = G(L)->mt[ttnov(obj)]; break;
    }
    if(mt != NULL) {
...
static const char *aux_upvalue(StkId fi, int n, TValue **val, CClosure **owner,
                               UpVal **uv) {
    switch(ttype(fi)) {
...
LUA_API void *lua_upvalueid(lua_State *L, int fidx, int n) {
    StkId fi = index2addr(L, fidx);
    switch(ttype(fi)) {
        case LUA_TLCL: { /* lua closure */ return *getupvalref(L, fidx, n, NULL);
        }
        case LUA_TCCL: { /* C closure */
            CClosure *f = clCvalue(fi);
