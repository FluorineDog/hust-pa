...
static Node *mainposition(const Table *t, const TValue *key) {
    switch(ttype(key)) {
        ...
        case LUA_TNUMFLT: return hashmod(t, l_hashfloat(fltvalue(key)));
        ...
        default: lua_assert(!ttisdeadkey(key)); return hashpointer(t, gcvalue(key));
    }
}
...
static void setnodevector(lua_State *L, Table *t, unsigned int size) {
    if(size == 0) {                        /* no elements to hash part? */
        ...
        if(lsize > MAXHBITS) luaG_runerror(L, "table overflow");
        size = twoto(lsize);
...
void luaH_resize(lua_State *L, Table *t, unsigned int nasize, unsigned int nhsize) {
    ...
    if(nasize < oldasize) { /* array part must shrink? */
        ...
        for(i = nasize; i < oldasize; i++) {
            if(!ttisnil(&t->array[i])) luaH_setint(L, t, i + 1, &t->array[i]);
        }
        /* shrink array */
...
void luaH_free(lua_State *L, Table *t) {
    if(!isdummy(t)) luaM_freearray(L, t->node, cast(size_t, sizenode(t)));
...
const TValue *luaH_getshortstr(Table *t, TString *key) {
    ...
    for(;;) { /* check whether 'key' is somewhere in the chain */
        ...
        if(ttisshrstring(k) && eqshrstr(tsvalue(k), key))
            ...
            int nx = gnext(n);
            if(nx == 0) return luaO_nilobject; /* not found */
            n += nx;
        }
    }
}
...
const TValue *luaH_get(Table *t, const TValue *key) {
    switch(ttype(key)) {
        ...
        default: return getgeneric(t, key);
    }
}
