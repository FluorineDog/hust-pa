        ...
        for(x = (sx - width / 2) / 32; x <= (sx + width / 2) / 32; x++) {
            for(i = ((x == (sx - width / 2) / 32) ? 0 : 3); i < 5; i++) {
                ...
                for(l = 0; l < 2; l++) {
                    ...
                    //
                    if(lpTile != NULL && iTileHeight > 0 &&
                       (dy + iTileHeight) * 16 + dh * 8 >= sy) {
                        //
                        // This tile may cover the sprite
                        //
                        PAL_AddSpriteToDraw(
                            lpTile, dx * 32 + dh * 16 - 16 - PAL_X(gpGlobals->viewport),
                            dy * 16 + dh * 8 + 7 + l + iTileHeight * 8 -
                                PAL_Y(gpGlobals->viewport),
                            iTileHeight * 8 + l);
                    }
                }
            }
...
--*/
{
    ...
    for(i = 0; i <= (short)gpGlobals->wMaxPartyMemberIndex + gpGlobals->nFollower; i++) {
        LPCBITMAPRLE lpBitmap = PAL_SpriteGetFrame(PAL_GetPlayerSprite((BYTE)i),
                                                   gpGlobals->rgParty[i].wFrame);
        ...
        //
        PAL_AddSpriteToDraw(
            lpBitmap, gpGlobals->rgParty[i].x - PAL_RLEGetWidth(lpBitmap) / 2,
    ...
    for(i = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex;
        ...
        lpFrame = PAL_SpriteGetFrame(
            lpSprite, lpEvtObj->wDirection * lpEvtObj->nSpriteFrames + iFrame);
...
--*/
{
    ...
    if(fWalking) {
        ...
        if(gpGlobals->g.PlayerRoles.rgwWalkFrames[gpGlobals->rgParty[0].wPlayerRole] ==
           4) {
            gpGlobals->rgParty[0].wFrame =
                gpGlobals->wPartyDirection * 4 + s_iThisStepFrame;
        } else {
            gpGlobals->rgParty[0].wFrame =
                gpGlobals->wPartyDirection * 3 + iStepFrameLeader;
        }
        ...
        for(i = 1; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++) {
            gpGlobals->rgParty[i].x =
                gpGlobals->rgTrail[1].x - PAL_X(gpGlobals->viewport);
            gpGlobals->rgParty[i].y =
                gpGlobals->rgTrail[1].y - PAL_Y(gpGlobals->viewport);

            if(i == 2) {
                gpGlobals->rgParty[i].x +=
                    (gpGlobals->rgTrail[1].wDirection == kDirEast ||
            ...
            //
            if(PAL_CheckObstacle(
                   PAL_XY(gpGlobals->rgParty[i].x + PAL_X(gpGlobals->viewport),
            ...
            if(gpGlobals->g.PlayerRoles
                   .rgwWalkFrames[gpGlobals->rgParty[i].wPlayerRole] == 4) {
                gpGlobals->rgParty[i].wFrame =
                    gpGlobals->rgTrail[2].wDirection * 4 + s_iThisStepFrame;
            } else {
                gpGlobals->rgParty[i].wFrame =
                    gpGlobals->rgTrail[2].wDirection * 3 + iStepFrameLeader;
            }
        }
        ...
        for(i = 1; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++) {
            int f =
                gpGlobals->g.PlayerRoles.rgwWalkFrames[gpGlobals->rgParty[i].wPlayerRole];
            if(f == 0) {
                f = 3;
            }
...
--*/
{
    ...
    if(g_InputState.dir != kDirUnknown) {
        xOffset =
            ((g_InputState.dir == kDirWest || g_InputState.dir == kDirSouth) ? -16 : 16);
        yOffset =
            ((g_InputState.dir == kDirWest || g_InputState.dir == kDirNorth) ? -8 : 8);
        ...
        if(!PAL_CheckObstacle(PAL_XY(xTarget, yTarget), TRUE, 0)) {
            ...
            // Move the viewport
            //
            gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + xOffset,
                                         PAL_Y(gpGlobals->viewport) + yOffset);
