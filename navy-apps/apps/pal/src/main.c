...
#else
    if(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_CDROM | SDL_INIT_NOPARACHUTE |
                SDL_INIT_JOYSTICK) == -1)
#endif
    {
...
--*/
{
    ...
    while(TRUE) {
        ...
        SDL_SetSurfacePalette(lpBitmapUp, gpScreen->format->palette);
#else
        SDL_SetPalette(lpBitmapDown, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(), 0,
                       256);
        SDL_SetPalette(lpBitmapUp, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(), 0, 256);
#endif
        ...
        if(iCraneFrame >= 1000 || g_InputState.dwKeyPress & (kKeyMenu | kKeySearch)) {
            ...
            if(dwTime < 15000) {
                ...
                while(dwTime < 15000) {
                    ...
                    SDL_SetSurfacePalette(lpBitmapUp, gpScreen->format->palette);
#else
                    SDL_SetPalette(lpBitmapDown, SDL_PHYSPAL | SDL_LOGPAL,
                                   VIDEO_GetPalette(), 0, 256);
                    SDL_SetPalette(lpBitmapUp, SDL_PHYSPAL | SDL_LOGPAL,
                                   VIDEO_GetPalette(), 0, 256);
#endif
                    UTIL_Delay(8);
                    dwTime += 250;
