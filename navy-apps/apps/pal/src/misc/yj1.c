...
static unsigned int get_bits(const void *src, unsigned int *bitptr, unsigned int count) {
...
static unsigned short get_loop(const void *src, unsigned int *bitptr,
...
static unsigned short get_count(const void *src, unsigned int *bitptr,
                                PYJ_1_BLOCKHEADER header) {
    unsigned short temp;
...
INT Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
    ...
    if(SWAP32(hdr->Signature) != 0x315f4a59) return -1;
    if(SWAP32(hdr->UncompressedLength) > (unsigned int)DestSize) return -1;
    ...
    for(i = 0; i < SWAP16(hdr->BlockCount); i++) {
        ...
        for(;;) {
            unsigned short loop;
            if((loop = get_loop(src, &bitptr, header)) == 0) break;
...
static unsigned char data1[0x100] = {
    };
static unsigned char data2[0x10] = {0x08, 0x05, 0x06, 0x04, 0x07, 0x05, 0x06, 0x03,
...
static void bit(void* data, unsigned int pos, int set) {
    BitField* bit = (BitField*)((unsigned char*)data + (pos >> 3));
    switch(pos & 0x7) {
        case 0: bit->b0 = set; break;
        case 1: bit->b1 = set; break;
        case 2: bit->b2 = set; break;
        case 3: bit->b3 = set; break;
        case 4: bit->b4 = set; break;
        case 5: bit->b5 = set; break;
        case 6: bit->b6 = set; break;
        case 7: bit->b7 = set; break;
    }
}
...
INT Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
    ...
    while(1) {
        ...
        if(tree.node[0x280].weight == 0x8000) {
            int i;
            for(i = 0; i < 0x141; i++)
                if(tree.list[i]->weight & 0x1) adjust_tree(tree, i);
