...
--*/
{
    ...
    xOffset =
        x * 32 + h * 16 - PAL_X(gpGlobals->viewport) - PAL_X(gpGlobals->partyoffset);
    ...
    while(xOffset != 0 || yOffset != 0) {
        ...
        xOffset =
            x * 32 + h * 16 - PAL_X(gpGlobals->viewport) - PAL_X(gpGlobals->partyoffset);
        yOffset =
            y * 16 + h * 8 - PAL_Y(gpGlobals->viewport) - PAL_Y(gpGlobals->partyoffset);
    }
...
--*/
{
    ...
    while(xOffset != 0 || yOffset != 0) {
        ...
        gpGlobals->rgTrail[0].wDirection = gpGlobals->wPartyDirection;
        gpGlobals->rgTrail[0].x =
            PAL_X(gpGlobals->viewport) + dx + PAL_X(gpGlobals->partyoffset);
        gpGlobals->rgTrail[0].y =
            PAL_Y(gpGlobals->viewport) + dy + PAL_Y(gpGlobals->partyoffset);
...
--*/
{
...
#endif
        " ",
        "\xA5\xBB\xB5\x7B\xA6\xA1\xAC\x4F\xA6\xDB\xA5\xD1\xB3\x6E\xC5\xE9\xA1\x41\xAB\xF6"
        "\xB7\xD3"
        " GNU General",
    ...
    while(rgszStrings[i][0] != '\0') {
        PAL_DrawText(rgszStrings[i], PAL_XY(25, 20 + i * 16), DESCTEXT_COLOR, TRUE,
                     FALSE);
        i++;
    }
...
--*/
{
    ...
    switch(pScript->wOperation) {
        ...
        case 0x0010:
            ...
            //
            if(!PAL_NPCWalkTo(wEventObjectID, pScript->rgwOperand[0],
                              pScript->rgwOperand[1], pScript->rgwOperand[2], 3)) {
                wScriptEntry--;
            }
            break;
        ...
        case 0x0011:
            ...
            if((wEventObjectID & 1) ^ (gpGlobals->dwFrameNum & 1)) {
                if(!PAL_NPCWalkTo(wEventObjectID, pScript->rgwOperand[0],
                                  pScript->rgwOperand[1], pScript->rgwOperand[2], 2)) {
                    wScriptEntry--;
                }
            } else {
                wScriptEntry--;
        ...
        case 0x0012:
            ...
            //
            pCurrent->x = pScript->rgwOperand[1] + PAL_X(gpGlobals->viewport) +
                          PAL_X(gpGlobals->partyoffset);
            pCurrent->y = pScript->rgwOperand[2] + PAL_Y(gpGlobals->viewport) +
        ...
        case 0x0018:
            ...
            if(gpGlobals->g.PlayerRoles.rgwEquipment[i][wEventObjectID] !=
        ...
        case 0x001C:
            ...
            if(pScript->rgwOperand[0]) {
                ...
                // Apply to one player. The wEventObjectID parameter should indicate the player role.
                //
                if(!PAL_IncreaseHPMP(wEventObjectID, 0,
                                     (SHORT)(pScript->rgwOperand[1]))) {
                    g_fScriptSuccess = FALSE;
                }
            }
        ...
        case 0x001D:
            ...
            if(pScript->rgwOperand[0]) {
                ...
                for(i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                    w = gpGlobals->rgParty[i].wPlayerRole;
                    PAL_IncreaseHPMP(w, (SHORT)(pScript->rgwOperand[1]),
                                     (SHORT)(pScript->rgwOperand[1]));
                }
            } else {
                //
        ...
        case 0x001F:
            ...
            //
            PAL_AddItemToInventory(pScript->rgwOperand[0],
                                   (SHORT)(pScript->rgwOperand[1]));
            break;
            ...
            if(!PAL_AddItemToInventory(
                ...
                for(i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                    ...
                    for(j = 0; j < MAX_PLAYER_EQUIPMENTS; j++) {
                        if(gpGlobals->g.PlayerRoles.rgwEquipment[j][w] ==
                           pScript->rgwOperand[0]) {
            ...
            if(pScript->rgwOperand[0]) {
                ...
                if(gpGlobals->g.PlayerRoles.rgwHP[wEventObjectID] == 0) {
                    gpGlobals->g.PlayerRoles.rgwHP[wEventObjectID] =
                        gpGlobals->g.PlayerRoles.rgwMaxHP[wEventObjectID] *
                        pScript->rgwOperand[1] / 10;
        ...
        case 0x0023:
            ...
            if(pScript->rgwOperand[1] == 0) {
                ...
                w = gpGlobals->g.PlayerRoles
                        .rgwEquipment[pScript->rgwOperand[1] - 1][iPlayerRole];
                if(w != 0) {
                    PAL_RemoveEquipmentEffect(iPlayerRole, pScript->rgwOperand[1] - 1);
                    PAL_AddItemToInventory(w, 1);
                    gpGlobals->g.PlayerRoles
                        .rgwEquipment[pScript->rgwOperand[1] - 1][iPlayerRole] = 0;
                }
            }
            break;
                        ...
                        if(j >= MAX_POISONS) {
                            for(j = 0; j < MAX_POISONS; j++) {
                                if(g_Battle.rgEnemy[i].rgPoisons[j].wPoisonID == 0) {
                                    g_Battle.rgEnemy[i].rgPoisons[j].wPoisonID =
                                        pScript->rgwOperand[1];
                                    g_Battle.rgEnemy[i].rgPoisons[j].wPoisonScript =
                                        PAL_RunTriggerScript(
                                            gpGlobals->g.rgObject[pScript->rgwOperand[1]]
                                                .poison.wEnemyScript,
                                            wEventObjectID);
                                    break;
                                }
                   ...
                   gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery) {
                    ...
                    if(j >= MAX_POISONS) {
                        for(j = 0; j < MAX_POISONS; j++) {
                            if(g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonID ==
                               0) {
                                g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonID =
                                    pScript->rgwOperand[1];
                                g_Battle.rgEnemy[wEventObjectID]
                                    .rgPoisons[j]
                                    .wPoisonScript = PAL_RunTriggerScript(
                                    gpGlobals->g.rgObject[pScript->rgwOperand[1]]
                                        .poison.wEnemyScript,
                                    wEventObjectID);
                                break;
        ...
        case 0x002D:
            ...
            //
            PAL_SetPlayerStatus(wEventObjectID, pScript->rgwOperand[0],
                                pScript->rgwOperand[1]);
            break;
        ...
        case 0x002E:
            ...
            if(RandomLong(0, i) >= gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery &&
               g_Battle.rgEnemy[wEventObjectID].rgwStatus[pScript->rgwOperand[0]] == 0) {
                g_Battle.rgEnemy[wEventObjectID].rgwStatus[pScript->rgwOperand[0]] =
                    pScript->rgwOperand[1];
            } else {
                wScriptEntry = pScript->rgwOperand[2] - 1;
            }
        ...
        case 0x0030:
            ...
            //
            {
                WORD *p =
                    (WORD *)(&gpGlobals
                                  ->rgEquipmentEffect[kBodyPartExtra]);    // HACKHACK
        ...
        case 0x0031:
            ...
            //
            gpGlobals->rgEquipmentEffect[kBodyPartExtra]
                .rgwSpriteNumInBattle[wEventObjectID] = pScript->rgwOperand[0];
            break;
        ...
        case 0x003F:
            ...
            //
            PAL_PartyRideEventObject(wEventObjectID, pScript->rgwOperand[0],
                                     pScript->rgwOperand[1], pScript->rgwOperand[2], 2);
            break;
        ...
        case 0x0043:
            ...
            PAL_PlayMUS(pScript->rgwOperand[0], (pScript->rgwOperand[0] != 0x3D),
                        pScript->rgwOperand[1]);
            break;
        ...
        case 0x0044:
            ...
            //
            PAL_PartyRideEventObject(wEventObjectID, pScript->rgwOperand[0],
                                     pScript->rgwOperand[1], pScript->rgwOperand[2], 4);
            break;
        ...
        case 0x0046:
            ...
            //
            {
                ...
                xOffset = ((gpGlobals->wPartyDirection == kDirWest ||
                            gpGlobals->wPartyDirection == kDirSouth)
                               ? 16
                               : -16);
                yOffset = ((gpGlobals->wPartyDirection == kDirWest ||
                            gpGlobals->wPartyDirection == kDirNorth)
                               ? 8
                               : -8);
        ...
        case 0x0051:
            ...
            VIDEO_UpdateScreen(NULL);
            PAL_FadeIn(
                gpGlobals->wNumPalette, gpGlobals->fNightPalette,
        ...
        case 0x0052:
            ...
            pEvtObj->sState *= -1;
            pEvtObj->sVanishTime =
                (pScript->rgwOperand[0] ? pScript->rgwOperand[0] : 800);
            break;
        ...
        case 0x0058:
            ...
            // in the inventory
            //
            if(PAL_GetItemAmount(pScript->rgwOperand[0]) <
               (SHORT)(pScript->rgwOperand[1])) {
                wScriptEntry = pScript->rgwOperand[2] - 1;
            }
            break;
        ...
        case 0x005E:
            ...
            for(i = 0; i < MAX_POISONS; i++) {
                if(g_Battle.rgEnemy[wEventObjectID].rgPoisons[i].wPoisonID ==
                   pScript->rgwOperand[0]) {
                    break;
                }
            }
        ...
        case 0x0064:
            ...
            //
            i = gpGlobals->g.rgObject[g_Battle.rgEnemy[wEventObjectID].wObjectID]
                    .enemy.wEnemyID;
        ...
        case 0x0065:
            ...
            //
            gpGlobals->g.PlayerRoles.rgwSpriteNum[pScript->rgwOperand[0]] =
                pScript->rgwOperand[1];
        ...
        case 0x0066:
            ...
            w += gpGlobals->g.PlayerRoles.rgwAttackStrength
                     [gpGlobals->rgParty[g_Battle.wMovingPlayerIndex].wPlayerRole];
            w += RandomLong(0, 4);
        ...
        case 0x006E:
            ...
            gpGlobals->viewport =
                PAL_XY(PAL_X(gpGlobals->viewport) + (SHORT)(pScript->rgwOperand[0]),
        ...
        case 0x0070:
            ...
            //
            PAL_PartyWalkTo(pScript->rgwOperand[0], pScript->rgwOperand[1],
        ...
        case 0x0074:
            ...
            for(i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                w = gpGlobals->rgParty[i].wPlayerRole;
                if(gpGlobals->g.PlayerRoles.rgwHP[w] <
                   gpGlobals->g.PlayerRoles.rgwMaxHP[w]) {
                    wScriptEntry = pScript->rgwOperand[0] - 1;
                    break;
                }
        ...
        case 0x0077:
            ...
            //
            PAL_PlayMUS(0, FALSE,
                        (pScript->rgwOperand[0] == 0)
                            ? 2.0f
                            : (FLOAT)(pScript->rgwOperand[0]) * 2);
        ...
        case 0x007F:
            ...
            if(pScript->rgwOperand[0] == 0 && pScript->rgwOperand[1] == 0) {
                ...
                gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + x,
                                             PAL_Y(gpGlobals->viewport) + y);
                ...
                do {
                    if(pScript->rgwOperand[2] == 0xFFFF) {
                        ...
                        gpGlobals->viewport = PAL_XY(pScript->rgwOperand[0] * 32 - 160,
                                                     pScript->rgwOperand[1] * 16 - 112);
                        ...
                        gpGlobals->partyoffset =
                            PAL_XY(PAL_X(gpGlobals->partyoffset) - x,
                                   PAL_Y(gpGlobals->partyoffset) - y);
        ...
        case 0x0081:
            ...
            //
            {
                if(pScript->rgwOperand[0] <=
                   ...
                   pScript->rgwOperand[0] >
                       gpGlobals->g.rgScene[gpGlobals->wNumScene].wEventObjectIndex) {
                    //
                ...
                x += ((gpGlobals->wPartyDirection == kDirWest ||
                       gpGlobals->wPartyDirection == kDirSouth)
                          ? 16
                          : -16);
                y += ((gpGlobals->wPartyDirection == kDirWest ||
                       gpGlobals->wPartyDirection == kDirNorth)
                          ? 8
                          : -8);
                ...
                if(abs(x) + abs(y * 2) < pScript->rgwOperand[1] * 32 + 16) {
                    if(pScript->rgwOperand[1] > 0) {
                        ...
                        //
                        pCurrent->wTriggerMode =
                            kTriggerTouchNormal + pScript->rgwOperand[1];
                    }
                } else {
                    wScriptEntry = pScript->rgwOperand[2] - 1;
                    g_fScriptSuccess = FALSE;
        ...
        case 0x0082:
            ...
            //
            if(!PAL_NPCWalkTo(wEventObjectID, pScript->rgwOperand[0],
                              pScript->rgwOperand[1], pScript->rgwOperand[2], 8)) {
                wScriptEntry--;
            }
            break;
        ...
        case 0x0086:
            ...
            for(i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                ...
                for(x = 0; x < MAX_PLAYER_EQUIPMENTS; x++) {
                    if(gpGlobals->g.PlayerRoles.rgwEquipment[x][w] ==
                       pScript->rgwOperand[0]) {
                        y = TRUE;
                        i = 999;
                        break;
        ...
        case 0x0090:
            ...
            //
            gpGlobals->g.rgObject[pScript->rgwOperand[0]]
                .rgwData[2 + pScript->rgwOperand[2]] = pScript->rgwOperand[1];
            break;
        ...
        case 0x0091:
            ...
            if(gpGlobals->fInBattle) {
                for(i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                    if(i != wEventObjectID &&
                       g_Battle.rgEnemy[i].wObjectID ==
                           g_Battle.rgEnemy[wEventObjectID].wObjectID) {
                        wScriptEntry = pScript->rgwOperand[0] - 1;
                        break;
                    }
                }
        ...
        case 0x0098:
            ...
            if(pScript->rgwOperand[0] > 0) {
                ...
                gpGlobals->rgParty[gpGlobals->wMaxPartyMemberIndex + 1].wPlayerRole =
                    pScript->rgwOperand[0];
            ...
            for(i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                if(w > 0 && g_Battle.rgEnemy[i].wObjectID == 0) {
                    ...
                    g_Battle.rgEnemy[i].wScriptOnReady =
                        g_Battle.rgEnemy[wEventObjectID].wScriptOnReady;
            ...
            for(i = 0; i < 10; i++) {
                for(j = 0; j <= g_Battle.wMaxEnemyIndex; j++) {
                    x = (PAL_X(g_Battle.rgEnemy[j].pos) +
                         PAL_X(g_Battle.rgEnemy[j].posOriginal)) /
                        2;
                    y = (PAL_Y(g_Battle.rgEnemy[j].pos) +
                         PAL_Y(g_Battle.rgEnemy[j].posOriginal)) /
                        2;
        ...
        case 0x009E:
            ...
            x = 0;
            w = pScript->rgwOperand[0];
            y = (((SHORT)(pScript->rgwOperand[1]) <= 0) ? 1
                                                        : (SHORT)pScript->rgwOperand[1]);
               ...
               g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusConfused] != 0) {
                ...
                for(i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                    if(g_Battle.rgEnemy[i].wObjectID == 0) {
                        ...
                        g_Battle.rgEnemy[i].e =
                            gpGlobals->g
                                .lprgEnemy[gpGlobals->g.rgObject[w].enemy.wEnemyID];
                        ...
                        g_Battle.rgEnemy[i].wScriptOnReady =
                            gpGlobals->g.rgObject[w].enemy.wScriptOnReady;
                        g_Battle.rgEnemy[i].flTimeMeter = int2F(50);
            ...
            if(g_Battle.iHidingTime <= 0 &&
               ...
               g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusConfused] == 0) {
                ...
                g_Battle.rgEnemy[wEventObjectID].e =
                    gpGlobals->g.lprgEnemy[gpGlobals->g.rgObject[pScript->rgwOperand[0]]
                                               .enemy.wEnemyID];
        ...
        case 0x00A1:
            ...
            for(i = 0; i < MAX_PLAYABLE_PLAYER_ROLES; i++) {
                gpGlobals->rgTrail[i].wDirection = gpGlobals->wPartyDirection;
                gpGlobals->rgTrail[i].x =
                    gpGlobals->rgParty[0].x + PAL_X(gpGlobals->viewport);
                gpGlobals->rgTrail[i].y =
                    gpGlobals->rgParty[0].y + PAL_Y(gpGlobals->viewport);
            }
        ...
        case 0x00A4:
            ...
            if(pScript->rgwOperand[0] == 68) {
                ...
                PAL_ScrollFBP(pScript->rgwOperand[0], pScript->rgwOperand[2],
                              pScript->rgwOperand[1]);
            }
#endif
            break;
    ...
    while(wScriptEntry != 0 && !fEnded) {
        ...
        switch(pScript->wOperation) {
            ...
            case 0x0004:
                ...
                PAL_RunTriggerScript(
                    pScript->rgwOperand[0],
                    ((pScript->rgwOperand[1] == 0) ? wEventObjectID
                                                   : pScript->rgwOperand[1]));
                wScriptEntry++;
                break;
            ...
            case 0x0005:
                ...
                if(PAL_DialogIsPlayingRNG()) {
                    ...
                    VIDEO_UpdateScreen(NULL);
                    UTIL_Delay((pScript->rgwOperand[1] == 0)
                                   ? 60
                                   : (pScript->rgwOperand[1] * 60));
                }
            ...
            case 0x0009:
                ...
                //
                {
                    ...
                    for(i = 0; i < (pScript->rgwOperand[0] ? pScript->rgwOperand[0] : 1);
                        i++) {
                        PAL_ProcessEvent();
                        while(SDL_GetTicks() < time) {
                            PAL_ProcessEvent();
            ...
            case 0x003E:
                ...
                //
                PAL_ClearDialog(TRUE);
                PAL_StartDialog(kDialogCenterWindow, (BYTE)pScript->rgwOperand[0], 0,
                                FALSE);
                wScriptEntry++;
                break;
...
begin:
    ...
    switch(pScript->wOperation) {
        ...
        case 0x0004:
            ...
            // Call subroutine
            //
            PAL_RunTriggerScript(pScript->rgwOperand[0], pScript->rgwOperand[1]
                                                             ? pScript->rgwOperand[1]
                                                             : wEventObjectID);
            wScriptEntry++;
            break;
        ...
        case 0xFFFF:
            ...
            if(wEventObjectID & (1 << 15)) {
                PAL_DrawText(PAL_GetMsg(pScript->rgwOperand[0]),
                             PAL_XY(75, iDescLine * 16 + 150), DESCTEXT_COLOR, TRUE,
                             FALSE);
            } else {
                PAL_DrawText(PAL_GetMsg(pScript->rgwOperand[0]),
                             PAL_XY(100, iDescLine * 16 + 3), DESCTEXT_COLOR, TRUE,
                             FALSE);
            }
            iDescLine++;
#endif
...
#ifdef PAL_WIN95
        case 0x00A7: wScriptEntry++; break;
#endif
