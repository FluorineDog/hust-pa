...
typedef struct tagEVENTOBJECT {
    ...
    USHORT
        nSpriteFramesAuto;    // total number of frames of the sprite, used by auto script
...
typedef struct tagSCENE {
    ...
    WORD
        wScriptOnTeleport;    // when teleporting out of this scene, execute script from here
    WORD
        wEventObjectIndex;    // event objects in this scene begins from number wEventObjectIndex + 1
} SCENE, *LPSCENE;
...
typedef struct tagGLOBALVARS {
    ...
    WORD wLayer;
    WORD
        wMaxPartyMemberIndex;    // max index of members in party (0 to MAX_PLAYERS_IN_PARTY - 1)
    ...
    POISONSTATUS rgPoisonStatus[MAX_POISONS]
                               [MAX_PLAYABLE_PLAYER_ROLES];    // poison status
