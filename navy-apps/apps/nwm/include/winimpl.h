...
class BgImage : public Window {
  public:
    BgImage(WindowManager *wm, int width, int height)
...
class WindowSwitcher : public Window {
    ...
    void project(Window *win, int basex, int basey, int w, int h) {
        ...
        for(int x = 0; x < w1; x++)
            for(int y = 0; y < h1; y++) {
                uint32_t col =
                    win->canvas[(y * win->h / h1) * win->w + (x * win->w / w1)];
                if(col & 0xff000000) {
                    col = border_col;
                }
    ...
    void sync() {
        if(visible) {
            ...
            for(Window *win : wm->windows)
                if(win) n++;    // number of windows

            int w1 = n * (2 * preview_pad + preview_px) + 2 * preview_border,
                base = (w - w1) / 2;
            ...
            for(int i = 0; i < w1; i++)
                for(int j = 0; j < h; j++) draw_px(base + i, j, 0xc4c4c4);
