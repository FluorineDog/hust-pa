...
typedef struct {
    ...
    float
        preferred_x;    // this determines where the cursor up/down tries to seek to along x
...
static void stb_textedit_click(STB_TEXTEDIT_STRING *str, STB_TexteditState *state,
                               float x, float y) {
...
static void stb_textedit_drag(STB_TEXTEDIT_STRING *str, STB_TexteditState *state, float x,
                              float y) {
...
static void stb_text_makeundo_delete(STB_TEXTEDIT_STRING *str, STB_TexteditState *state,
                                     int where, int length);
static void stb_text_makeundo_insert(STB_TexteditState *state, int where, int length);
static void stb_text_makeundo_replace(STB_TEXTEDIT_STRING *str, STB_TexteditState *state,
...
static void stb_textedit_delete(STB_TEXTEDIT_STRING *str, STB_TexteditState *state,
                                int where, int len) {
...
retry:
    switch(key) {
        ...
        case STB_TEXTEDIT_K_INSERT: state->insert_mode = !state->insert_mode; break;
#endif
        ...
        case STB_TEXTEDIT_K_LINESTART:
            ...
            if(state->single_line)
                ...
                while(state->cursor > 0 && STB_TEXTEDIT_GETCHAR(str, state->cursor - 1) !=
    ...
    if(state->undo_point > 0) {
        ...
        if(state->undo_rec[0].char_storage >= 0) {
            ...
            STB_TEXTEDIT_memmove(
                ...
                (size_t)((size_t)state->undo_char_point * sizeof(STB_TEXTEDIT_CHARTYPE)));
            for(i = 0; i < state->undo_point; ++i)
                if(state->undo_rec[i].char_storage >= 0)
                    state->undo_rec[i].char_storage =
                        state->undo_rec[i].char_storage -
                        (short)
                            n;    // vsnet05 // @OPTIMIZE: get rid of char_storage and infer it
        }
        --state->undo_point;
        STB_TEXTEDIT_memmove(
            state->undo_rec, state->undo_rec + 1,
            (size_t)((size_t)state->undo_point * sizeof(state->undo_rec[0])));
    }
}
    ...
    if(state->redo_point <= k) {
        ...
        if(state->undo_rec[k].char_storage >= 0) {
            ...
            STB_TEXTEDIT_memmove(
                ...
                (size_t)((size_t)(STB_TEXTEDIT_UNDOCHARCOUNT - state->redo_char_point) *
        ...
        STB_TEXTEDIT_memmove(
            state->undo_rec + state->redo_point, state->undo_rec + state->redo_point - 1,
