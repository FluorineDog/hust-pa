...
fix16_t fix16_sin(fix16_t inAngle) {
    ...
    if(tempAngle < 0) tempAngle += (fix16_pi << 1);

    fix16_t tempOut;
    if(tempAngle >= fix16_pi) {
        tempAngle -= fix16_pi;
        if(tempAngle >= (fix16_pi >> 1)) tempAngle = fix16_pi - tempAngle;
        tempOut =
            -(tempAngle >= _fix16_sin_lut_count ? fix16_one : _fix16_sin_lut[tempAngle]);
    } else {
        if(tempAngle >= (fix16_pi >> 1)) tempAngle = fix16_pi - tempAngle;
        tempOut =
            (tempAngle >= _fix16_sin_lut_count ? fix16_one : _fix16_sin_lut[tempAngle]);
...
fix16_t fix16_asin(fix16_t x) {
    if((x > fix16_one) || (x < -fix16_one)) return 0;
...
fix16_t fix16_atan2(fix16_t inY, fix16_t inX) {
    ...
    hash ^= hash >> 20;
    hash &= 0x0FFF;
    if((_fix16_atan_cache_index[0][hash] == inX) &&
