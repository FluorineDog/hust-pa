...
void cpu_address_relative() {
    ...
    if(flag < 0 || flag > 0) op_address -= 0x100;
    op_address += cpu.PC;
...
void cpu_address_indirect_x() {
    ...
    op_address = (memory_readb((arg_addr + cpu.X + 1) & 0xFF) << 8) |
                 memory_readb((arg_addr + cpu.X) & 0xFF);
    op_value = memory_readb(op_address);
    cpu.PC++;
}
...
void cpu_address_indirect_y() {
    byte arg_addr = memory_readb(cpu.PC);
    op_address =
        (((memory_readb((arg_addr + 1) & 0xFF) << 8) | memory_readb(arg_addr)) + cpu.Y) &
        0xFFFF;
    op_value = memory_readb(op_address);
    cpu.PC++;
