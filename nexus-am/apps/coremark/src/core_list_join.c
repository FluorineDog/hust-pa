...
ee_s16 calc_func(ee_s16 *pdata, core_results *res) {
    ...
    ee_s16 retval;
    ee_u8 optype =
        (data >> 7) & 1; /* bit 7 indicates if the function result has been cached */
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed) {
    ...
    ee_u32 size =
        (blksize / per_item) -
        2; /* to accomodate systems with 64b pointers, and make sure same code is executed, set max list elements */
    ...
    for(i = 0; i < size; i++) {
        ...
        info.data16 =
            (dat << 8) |
            dat; /* fill the data with actual data and upper bits with rebuild value */
    ...
    while(finder->next != NULL) {
        if(i < size / 5) /* first 20% of the list in order */
            ...
            finder->info->idx =
                0x3fff &
                (((i & 0x07) << 8) |
                 pat); /* make sure the mixed items end up after the ones in sequence */
        }
        finder = finder->next;
    }
    list = core_list_mergesort(list, cmp_idx, NULL);
