...
static size_t qlz_compress_core(const unsigned char *source, unsigned char *destination,
    ...
    const unsigned char *last_matchstart =
        last_byte - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END;
    ui32 fetch = 0;
    unsigned int lits = 0;
            ...
            if(offset2 < src - MINOFFSET && c > 0 &&
               ((fast_read(offset2, 3) ^ fetch) & 0xffffff) == 0) {
                ...
                if(*(offset2 + matchlen) == *(src + matchlen)) {
                    matchlen = 4;
                    while(*(offset2 + matchlen) == *(src + matchlen) &&
                          matchlen < remaining)
                        matchlen++;
                }
            } else
...
size_t qlz_compress(const void *source, char *destination, size_t size,
...
#endif
    {
        ...
        r = base +
            qlz_compress_core(src, (unsigned char *)destination + base, size, state);
    ...
    *destination |= (1 << 6);
    *destination |=
        ((QLZ_STREAMING_BUFFER == 0 ? 0
                                    : (QLZ_STREAMING_BUFFER == 100000
                                           ? 1
...
size_t qlz_decompress(const char *source, void *destination,
...
#endif
    {
        if((*source & 1) == 1) {
            ...
            dsiz = qlz_decompress_core((const unsigned char *)source,
